...
void ViewManager::ProcessKeyboardEvents()
{
	...
	// camera moves upward when Q key is pressed
	if (glfwGetKey(m_pWindow, GLFW_KEY_Q) == GLFW_PRESS)
	{
		g_pCamera->Position += g_pCamera->Up * gDeltaTime * g_pCamera->MovementSpeed;
	}

	// camera moves downward when Q key is pressed
	if (glfwGetKey(m_pWindow, GLFW_KEY_E) == GLFW_PRESS)
	{
		g_pCamera->Position -= g_pCamera->Up * gDeltaTime * g_pCamera->MovementSpeed;
	}

	// persepctive display when P key is pressed
	if (glfwGetKey(m_pWindow, GLFW_KEY_P) == GLFW_PRESS)
	{
		bOrthographicProjection = false;
	}

	// orthographic display when O key is pressed
